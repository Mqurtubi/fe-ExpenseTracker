...
export default function TransactionTableItem({
  ...
  return (
    <tr className="border-b border-slate-300 ">
      ...
      <td className="px-5 py-2">
        <span
          className={`font-semibold ${data.type === "EXPENSE" ? "text-red-800 bg-red-300/50" : "text-green-800 bg-green-300/50"} py-1 px-2 rounded-xl`}
        >
          {data.type}
        </span>
      </td>
      <td
        className={`${data.type === "EXPENSE" ? "text-red-700" : "text-green-700"} font-semibold px-5 py-2`}
      >
        {data.type === "EXPENSE" ? "-" : "+"} Rp{" "}
